...
`# Example from YOLOv5class ModelVariant(Enum):    YOLOV5N = "yolov5n"    YOLOV5S = "yolov5s"    YOLOV5M = "yolov5m"    YOLOV5L = "yolov5l"    YOLOV5X = "yolov5x" # Example from Llama3class ModelVariant(Enum):    LLAMA_3_2_1B = "meta-llama/Llama-3.2-1B"    LLAMA_3_2_1B_INSTRUCT = "meta-llama/Llama-3.2-1B-Instruct"    LLAMA_3_8B = "meta-llama/Meta-Llama-3-8B"    # ...`
...
`# From forge/test/models/pytorch/vision/resnet/test_resnet.py:34-42module_name = record_model_properties(    framework=Framework.PYTORCH,      # Framework enum    model=ModelArch.RESNET,            # ModelArch enum    variant="50",                      # String variant    source=Source.HUGGINGFACE,         # Source enum    task=Task.CV_IMAGE_CLASSIFICATION, # Task enum    group=ModelGroup.RED,              # Optional: test grouping    priority=ModelPriority.P1,         # Optional: priority level)`
...
`# test_yolov5_320x320, test_yolov5_640x640, test_yolov5_480x480, test_yolov5_1280x1280variants = [ModelVariant.YOLOV5N, ModelVariant.YOLOV5S, ModelVariant.YOLOV5M,             ModelVariant.YOLOV5L, ModelVariant.YOLOV5X] @pytest.mark.nightly@pytest.mark.parametrize("variant", variants)def test_yolov5_640x640(restore_package_versions, variant):    # Load with specific resolution    ims, n, files, shape0, shape1, input_tensor = loader.load_inputs(        dtype_override=torch.bfloat16,         input_size=640  # Resolution specified here    )`
...
`# Causal LM variants (test_llama3.py:34-69)CausalLMVariant.LLAMA_3_2_1B           # PassesCausalLMVariant.LLAMA_3_2_1B_INSTRUCT  # PassesCausalLMVariant.LLAMA_3_8B              # xfail: multi-chipCausalLMVariant.LLAMA_3_1_8B            # xfail: multi-chip # Sequence classification variants (test_llama3.py:126-157)SequenceClassificationVariant.LLAMA_3_2_1B           # PassesSequenceClassificationVariant.LLAMA_3_2_1B_INSTRUCT  # PassesSequenceClassificationVariant.LLAMA_3_8B              # xfail: multi-chip`
**TextModelWrapper usage pattern for LLM models:**

`# Required for most LLM models to handle embeddingsfrom test.models.models_utils import TextModelWrapper loader = CausalLMLoader(variant)model = loader.load_model()framework_model = TextModelWrapper(    model=model,     text_embedding=model.model.embed_tokens  # Extract embedding layer)framework_model.eval()`
...
`# LLaVA wrapper (forge/test/models/pytorch/multimodal/llava/test_llava.py:26-34)class Wrapper(torch.nn.Module):    def __init__(self, model):        super().__init__()        self.model = model     def forward(self, input_ids, attention_mask, pixel_values):        inputs = {            "input_ids": input_ids,             "attention_mask": attention_mask,             "pixel_values": pixel_values        }        output = self.model(**inputs)        return output.logits  # Extract only logits # Stable Diffusion transformer wrapper (test_stable_diffusion_v35.py:27-43)class StableDiffusionWrapper(torch.nn.Module):    def __init__(self, model, joint_attention_kwargs=None, return_dict=False):        super().__init__()        self.model = model        self.joint_attention_kwargs = joint_attention_kwargs        self.return_dict = return_dict     def forward(self, latent_model_input, timestep, prompt_embeds, pooled_prompt_embeds):        noise_pred = self.model(            hidden_states=latent_model_input,            timestep=timestep,            encoder_hidden_states=prompt_embeds,            pooled_projections=pooled_prompt_embeds,            joint_attention_kwargs=self.joint_attention_kwargs,            return_dict=self.return_dict,        )[0]        return noise_pred # Phi3.5 Vision wrapper (test_phi3_5_vision.py:25-31)class Wrapper(torch.nn.Module):    def __init__(self, model):        super().__init__()        self.model = model     def forward(self, input_ids, attention_mask, pixel_values, image_sizes):        return self.model(input_ids, attention_mask, None, None, None, pixel_values, image_sizes)`
...
`# From forge/test/models/onnx/timeseries/test_nbeats_onnx.py:56-65# Step 1: Load PyTorch modeltorch_model = NBeatsWithSeasonalityBasis(...)torch_model.eval()inputs = [x.contiguous(), x_mask.contiguous()] # Step 2: Export to ONNXonnx_path = f"{forge_tmp_path}/{variant}.onnx"torch.onnx.export(    torch_model,     (inputs[0], inputs[1]),  # Tuple of input tensors    onnx_path,     opset_version=17) # Step 3: Load ONNX and wraponnx_model = onnx.load(onnx_path)onnx.checker.check_model(onnx_model)framework_model = forge.OnnxModule(module_name, onnx_model) # Step 4: Compile and verifycompiled_model = forge.compile(onnx_model, inputs, module_name=module_name)verify(inputs, framework_model, compiled_model)`
...
`# From test/models/models_utils.py (exact implementation not shown but usage is clear)from test.models.models_utils import TextModelWrapper # Standard pattern for causal LM modelsloader = CausalLMLoader(variant)model = loader.load_model()framework_model = TextModelWrapper(    model=model,     text_embedding=model.model.embed_tokens  # Pass embedding layer)framework_model.eval() # For sequence/token classification (no embedding needed)framework_model = TextModelWrapper(model=model)framework_model.eval()`
**Example wrapper implementations:**

`# Falcon wrapper (test_falcon.py:44-50)class Wrapper(torch.nn.Module):    def __init__(self, model):        super().__init__()        self.model = model     def forward(self, input_ids, attention_mask):        # Drops past_key_values, passes None for position_ids        return self.model(input_ids, None, attention_mask) # LLaVA wrapper (test_llava.py:26-34)class Wrapper(torch.nn.Module):    def __init__(self, model):        super().__init__()        self.model = model     def forward(self, input_ids, attention_mask, pixel_values):        # Converts positional args to dict, extracts logits        inputs = {            "input_ids": input_ids,             "attention_mask": attention_mask,             "pixel_values": pixel_values        }        output = self.model(**inputs)        return output.logits # Phi3.5 Vision wrapper (test_phi3_5_vision.py:25-31)class Wrapper(torch.nn.Module):    def __init__(self, model):        super().__init__()        self.model = model     def forward(self, input_ids, attention_mask, pixel_values, image_sizes):        # Passes None for unused args (position_ids, past_key_values, inputs_embeds)        return self.model(input_ids, attention_mask, None, None, None, pixel_values, image_sizes)`
