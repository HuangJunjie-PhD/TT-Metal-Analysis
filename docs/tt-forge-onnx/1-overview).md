...
deepwiki: https://deepwiki.com/tenstorrent/tt-forge-onnx/1-overview)
---
...
`import torchimport forge class Add(torch.nn.Module):    def forward(self, a, b):        return a + b a = torch.rand(2, 32, 32)b = torch.rand(2, 32, 32) framework_model = Add()compiled_model = forge.compile(framework_model, sample_inputs=[a, b])out = compiled_model(a, b)`
