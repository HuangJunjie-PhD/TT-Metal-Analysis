...
graph TB
    subgraph "Application Layer"
        TTNN_OP["ttnn::operations::unary::detail::unary_impl"]
    end
    
    subgraph "LLK API & SFPU Logic"
        UNARY_UTILS["unary_op_utils.cpp<br/>get_op_init_and_func"]
        SFPU_MACROS["llk_math_eltwise_unary_sfpu_macros.h"]
        SFPU_TYPES["SfpuType Enum"]
    end
    
    subgraph "Math/Compute Headers"
        MathCommon["llk_math_common_api.h"]
        SFPU_Headers["ckernel_sfpu_trigonometry.h"]
        SFPU_EXP["ckernel_sfpu_exp.h"]
    end
    
    subgraph "Hardware Execution"
        TRISC["TRISC (Compute)<br/>Unpack/Math/Pack threads"]
        SFPU_HW["SFPU Hardware Unit"]
    end
    
    TTNN_OP --> UNARY_UTILS
    UNARY_UTILS --> SFPU_MACROS
    SFPU_MACROS --> SFPU_TYPES
    SFPU_TYPES --> SFPU_Headers
    SFPU_Headers --> SFPU_HW
    TRISC --> SFPU_HW
```
...
```mermaid
graph LR
    subgraph "TTNN Operation Space"
        TTNN_OP["ttnn::abs"]
        OP_TYPE["UnaryOpType::ABS"]
    end

    subgraph "LLK Math API Space"
        MACRO_DEF["SFPU_OP_COMPUTE_KERNEL_API_INCLUDE"]
        SFPU_ENUM["SfpuType::abs"]
    end

    subgraph "Hardware ISA Space"
        SFPI_VFLOAT["sfpi::vFloat"]
        SFPU_HW["SFPU Hardware Unit"]
    end
...
```mermaid
graph TB
    subgraph "SFPI_Toolchain_Binaries"
        GPP["riscv-tt-elf-g++"]
    end

    subgraph "Processor_Classes"
        BRISC["brisc"]
        NCRISC["ncrisc"]
        TRISC["trisc=0,1,2"]
        ERISC["aerisc/main_aerisc"]
        DRISC["drisc (Blackhole Only)"]
    end
...
Sources: [tt_metal/hw/CMakeLists.txt:2-18](), [tt_metal/hw/toolchain/erisc-b0-kernel.ld:19-22](), [tt_metal/hw/toolchain/erisc-b0-app-sections.ld:11-14]()
```
