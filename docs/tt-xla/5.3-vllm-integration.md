...
graph TB
    ...
    subgraph "TT Plugin Package - integrations/vllm_plugin/"
        SetupPy["setup.py<br/>entry_points"]
        InitPy["__init__.py<br/>register()"]
        TTPlatform["platform.py<br/>TTPlatform"]
        TTConfig["platform.py<br/>TTConfig"]
    end
    
    VllmEngine --> PluginRegistry
    PluginRegistry -->|"discover entry point<br/>vllm.platform_plugins"| SetupPy
    SetupPy -->|"calls register()"| InitPy
    InitPy -->|"returns class path<br/>vllm_tt.platform.TTPlatform"| PluginRegistry
    PluginRegistry --> PlatformInterface
    PlatformInterface -->|"instantiate"| TTPlatform
    TTPlatform --> TTConfig
    
    style SetupPy fill:#f9f9f9,stroke:#333,stroke-width:2px
    style InitPy fill:#f9f9f9,stroke:#333,stroke-width:2px
...
### Related: Module Replacement Example
...
### Related: ComparisonResult Processing
