...
`// Wormhole B0: Required separate instructionsSFPXOR VA, LReg[SignBit], LReg[TempA]  // Negate VASFPMAD TempA, VB, VC, VD // Blackhole A0: Single instructionTT_SFPMAD(VA, VB, VC, VD, SFPMAD_MOD1_NEGATE_VA)`
...
`uint10_t Adj16(uint10_t r) {  if (Config.DEST_ACCESS_CFG_remap_addrs) {    r = (r & 0x3c7) ^ ((r & 0x030) >> 1) ^ ((r & 0x008) << 2);  }  return r;} uint10_t Adj32(uint10_t r) {  r = Adj16(r);  if (Config.DEST_ACCESS_CFG_swizzle_32b) {    r = (r & 0x3f3) ^ ((r & 0x018) >> 1) ^ ((r & 0x004) << 1);  }  return ((r & 0x1f8) << 1) | (r & 0x207);}`
