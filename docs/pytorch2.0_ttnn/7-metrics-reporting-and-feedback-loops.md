...
`csv_header_mappings = {    "model": ("Model", "Name of the model."),    "success": ("Status", "✅/🚧/❌ indicator"),    "batch_size": ("Batch", "Batch size used"),    # ... more mappings}`
...
`class AtenOpTestExporter(InputVarPerOp):    def export_tests(self, template_dir, basedir, model_name, model_name_):        for opname, inputs_variations in sort_by_opname.items():            inputs_strings = [_unjoin_br(input_variations)                             for input_variations in inputs_variations.keys()]            # Apply template and write test file`
Templates used:
...
`def get_op_blocklist(self, threshold: list):    op_blocklist = {}    for model_name, input_vars in self.input_vars_result.items():        for input_var in input_vars:            if need_block(input_var, threshold):                opname = input_var["opname"]                if opname not in op_blocklist:                    op_blocklist[opname] = []                op_blocklist[opname].append(input_var["input_strings"])    return op_blocklist`
...
`# Blocklistsaten_add_Tensor_blocklist = [    ["Tensor<[1, 2, 3]> self = ?", "Tensor<[4, 5, 6]> other = ?"],    # ... more blocked variations] # Guard functiondef guard_aten(blocklist, node):    inputs = get_inputs(node)    inputs_str = [str(i) for i in inputs]    if inputs_str in blocklist:        return False  # Block this variation    return True # Guard mappingGUARD = {    torch.ops.aten.add.Tensor: partial(guard_aten, aten_add_Tensor_blocklist),    # ... more guards}`
