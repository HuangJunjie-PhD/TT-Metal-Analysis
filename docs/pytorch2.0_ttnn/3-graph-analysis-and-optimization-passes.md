...
`foldable_ops = {    torch.ops.aten.lift_fresh_copy.default,    torch.ops.aten.pow.Tensor_Tensor,    torch.ops.aten.arange.start,    torch.ops.aten.unsqueeze.default,    torch.ops.aten.view.default,    torch.ops.aten.mul.Tensor,    torch.ops.aten.cumsum.default,    # ... (see full list in code)}`
### Folding Process
...
`def get_sharded_size(concat_dim_size, node):    num_devices = self.device.get_num_devices()    shard_dim_size = concat_dim_size / node.meta["concat_size"]    num_shards_per_device = math.ceil(node.meta["concat_size"] / num_devices)    return int(shard_dim_size * num_shards_per_device)`
...
`# Create mocked input for preprocessingmocked_input_node = (    False,    "ttnn.ones",    (),    {"shape": tensor_shape, "device": TtnnDevice(),      "dtype": spec_dtype(), "layout": TtnnTileLayout()}) # Rewrite args to use RunOnceIdxnew_args[1] = pickle.dumps(weight_node_idx)  # weightnew_args[2] = pickle.dumps(bias_node_idx)    # bias # Set return_weights_and_bias flagnew_kwargs["return_weights_and_bias"] = True`
...
`with graph.inserting_after(node):    new_node = graph.call_function(deallocate, args=(n,))`
...
`class PassBase:    def call(self, gm: torch.fx.GraphModule) -> PassResult:        # Transform gm.graph        modified = True/False        return PassResult(gm, modified)`
