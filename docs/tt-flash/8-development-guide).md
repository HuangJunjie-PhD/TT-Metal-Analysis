...
`# setup.py structureimport tomli # Read version and metadata from pyproject.tomlwith open("pyproject.toml", "rb") as f:    toml_data = tomli.load(f) setup(    name="tt-flash",    version=toml_data['project']['version'],    packages=find_packages(),    python_requires=">=3.10",    entry_points={        'console_scripts': [            'tt-flash = tt_flash:main',        ]    },)`
...
`# Activate development environmentsource .env/bin/activate # Format all Python filesblack tt_flash/ # Check formatting without changesblack --check tt_flash/`
...
`# Activate development environmentsource .env/bin/activate # Run type checking on the codebasebasedpyright tt_flash/`
...
`git clone https://github.com/tenstorrent/tt-flash.gitcd tt-flashmake build`
...
`black tt_flash/basedpyright tt_flash/`
6.   **Production testing**:

`make releasesource my-env/bin/activatett-flash --help`
...
`# From pyproject.toml"tomli == 2.0.1; python_version < '3.11'""importlib-metadata >= 1.4.0; python_version < '3.8'""importlib-resources >= 1.3.0; python_version < '3.9'"`
...
`# Activate development environmentsource .env/bin/activate # Run tt-flash with modified codett-flash --help # Test flashing workflow (requires hardware)tt-flash flash path/to/bundle.fwbundle # Test verificationtt-flash verify`
### Installing Additional Dependencies

`# Activate development environmentsource .env/bin/activate # Install additional dev toolspip install <package-name>`
### Updating Version
...
`# Use specific Python versionPYTHON=/usr/bin/python3.11 make build # Point to custom luwen development directoryLUWEN_DIR=/path/to/luwen make build`
