...
`// Example: Matrix Unit operations with 5-cycle latencyTT_MVMUL(0, 0, ADDR_MOD_0);  // Cycle 0: IssueTT_MVMUL(1, 0, ADDR_MOD_0);  // Cycle 1: Issue (independent)TT_MVMUL(2, 0, ADDR_MOD_0);  // Cycle 2: Issue (independent)TT_MVMUL(3, 0, ADDR_MOD_0);  // Cycle 3: Issue (independent)TT_MVMUL(4, 0, ADDR_MOD_0);  // Cycle 4: Issue (independent)// Cycle 5: First MVMUL completesTT_MVMUL(5, 0, ADDR_MOD_0);  // Can consume result from first MVMUL`
...
`TT_SFPMAD(1, 2, 3, 4, 0);  // Write to LReg[4]TT_SFPNOP();               // Required delayTT_SFPLOAD(4, ...);        // Can now read LReg[4]`
...
`TT_MOVB2A(...);           // Move from SrcB to SrcA// Next cycle: Only MOVD2A or MOVB2A allowed// Hardware automatically stalls other instructions`
...
`// Pseudo-code structurefor (int tile = 0; tile < num_tiles; tile++) {    // RISCV B: Issue NoC transfer for tile N+1    noc_async_read(tile + 1);        // RISCV T0/T1/T2: Process tile N    unpack_phase();    math_phase();    pack_phase();        // Wait for NoC transfer of tile N+1 to complete    noc_wait();}`
