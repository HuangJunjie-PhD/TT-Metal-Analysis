...
deepwiki: https://deepwiki.com/tenstorrent/pytorch2.0_ttnn
---
...
`device = ttnn.open_device(device_id=0)option = TorchTtnnOption(device=device)`
For multi-device data parallel execution:

`device = ttnn.open_mesh_device(ttnn.MeshShape(1, 2))  # 1x2 meshoption = TorchTtnnOption(device=device, data_parallel=True)`
...
`class ConversionStatus(Enum):    DONE      # Successfully converted to TT-NN    FALLBACK  # Partially converted (some inputs fail)    NONE      # No conversion applied    REMOVED   # Operation removed from graph    UNKNOWN   # Status not yet determined`
...
`git clone https://github.com/tenstorrent/pytorch2.0_ttnn.gitcd pytorch2.0_ttnnpip install -e .`
